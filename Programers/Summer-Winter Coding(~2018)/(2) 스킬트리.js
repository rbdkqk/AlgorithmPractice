...
/* 기록해 둘 사항 
  ...
  (2) 주어진 스킬트리 중, 선행스킬과 1개만 연관있다면 통과되어야 하는데, 이 부분이 아직 구현되지 않음.
    => 다시 보니, 제일 선행스킬의 제일 첫번째 것 하나만 포함되었다면 통과되어야 맞는데(이 부분은 구현됨, 위 예시에서 C),
        선행스킬의 0번째 값이 아닌 다른 스킬이라면, 그것이 단 한개만 들어갔더라도 틀려야 맞음. 
        (위 예에서 B, 이것 하나만 들어갔다면, B는 선행스킬인 C 없이는 찍을 수 없으므로, 틀린 답임.)
